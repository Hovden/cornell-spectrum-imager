 ...
 */
public abstract class CSI_SpectrumData implements MouseListener, MouseMotionListener, Measurements, KeyListener {

	/**
	 * 
	 */
	CSI_Spectrum_Analyzer csi_Spectrum_Analyzer;
	...
	CSI_SpectrumData(CSI_Spectrum_Analyzer csi_Spectrum_Analyzer, ImagePlus img)
	{
	...
	void updateProfile() {
		...
		if (pwin == null) {
			...
			pwin.addComponentListener(new CSI_ResizeListener(this));
		}
		pwin.drawPlot(plot);
	...
	void setFit(int fitType) {
		...
		case CONSTANT_FIT: {
			fit = new CSI_ConstantFit();
			break;
		}
		case LINEAR_FIT: {
			fit = new CSI_LinearFit();
			break;
		}
		case EXPONENTIAL_FIT: {
			fit = new CSI_ExponentialFit();
			break;
		}
		case POWER_FIT: {
			fit = new CSI_PowerFit();
			break;
		}
		case LCPL_FIT: {
			fit = new CSI_LCPLFit();
