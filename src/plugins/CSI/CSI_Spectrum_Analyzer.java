...
public class CSI_Spectrum_Analyzer implements PlugInFilter {
	...
	public void run(ImageProcessor ip) { // this function runs upon opening
		...
		if (img.getStackSize() < 2) {
			if (img.getHeight() < 2) {
				state = new SpectrumData0D(); // Spectrum data is single
												// spectrum
			} else
	...
	private class TestListener implements ActionListener, ItemListener, MouseListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (b == miDoc) { // Display 'About' information
				...
				if (!isCalibrating) { // If not currently calibrating
					isCalibrating = true; // set twoppointcalibration mode to
											// true
					...
					if (!twoptcalib) { // If currently calibrating, but not
						removeCalibrateSliders(panSliders, panButtons); // in
																		// two
																		// point
																		// mode,
																		// then
						...
						addCalibrateSliders(panSliders, panButtons); // and
																		// replace
																		// with
																		// two
																		// point
				...
				if (!isCalibrating) { // If not currently calibrating
					isCalibrating = true; // set onepointcalibration mode to
											// true
				...
				if (comFit.getSelectedItem().equals("No Fit")) {
					...
					state.fitToModel(state.X0, state.X1, state.iX0, state.iX1).show(); // integrate
																						// data
				System.gc();
			} else if (b == butPCA) {
				if (weightedPCA)
			...
			} else if (b == butCancelCalibration) {// If cancel calibration
													// button was clicked
				isCalibrating = false; // Turn calibrating state to off
				removeCalibrateSliders(panSliders, panButtons); // remove
																// calibration
																// GUI elements
				state.pwin.pack();
				state.updateProfile();
		...
		public void itemStateChanged(ItemEvent e) {
			...
			} else if (b == comFit) { // If combo box (drop-down menu) is
										// clicked
	...
	private class ScrollListener implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			...
			if (s == sldOffset) { // The offset slider value changed
			...
			} else if (s == sldLeft) { // The background position slider value
										// changed
			...
			} else if (s == sldWidth) { // The background width slider value
										// changed
				sldIWidth.repaint();
				if (sldLeft.getValue() + sldWidth.getValue() >= state.size) {
					sldWidth.setValue(state.size - sldLeft.getValue() - 1);
				...
				state.X1 = sldLeft.getValue() + sldWidth.getValue();
				txtWidth.setText(String.format("%.1f", state.x[state.X1] - state.x[state.X0]));
			} else if (s == sldILeft) { // The integration position slider value
										// changed
			...
			} else if (s == sldIWidth) { // The integration width slider value
											// changed
				if (sldILeft.getValue() + sldIWidth.getValue() >= state.size) {
					sldIWidth.setValue(state.size - sldILeft.getValue() - 1);
					sldIWidth.repaint();
				...
				state.iX1 = sldILeft.getValue() + sldIWidth.getValue();
				txtIWidth.setText(String.format("%.1f", state.x[state.iX1] - state.x[state.iX0]));
			} else if (s == sldCLeft) { // The left calibration slider value
										// changed
				if (twoptcalib) {
			...
			} else if (s == sldCRight) { // The right calibration slider value
											// changed
				if (twoptcalib) {
	...
	private class LCPLFit extends Fit {
		...
		@Override
		Matrix createFit(double[] x, Matrix y, int start, int end) {
			double[] powerLawCoeffs = (new PowerFit()).createFit(x, y, start, end).toDoubleArray()[1];
	...
	private class ModelFit {
		...
		void createModelNoG(double[] x, Matrix y, int bStart, int bEnd, int eStart, int eEnd, Fit fit) {
	...
	private class SpectrumData2D extends SpectrumData {
		...
		ImagePlus HCMintegrate(int fitStart, int fitEnd, int intStart, int intEnd) {
			...
			ImagePlus imgint = new ImagePlus(
					img.getTitle() + " HCM integrated from " + String.format("%.1f", state.x[intStart]) + " "
