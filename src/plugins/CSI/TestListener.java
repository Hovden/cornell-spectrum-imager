 ...
 */
class TestListener implements ActionListener, ItemListener, MouseListener {
	 ...
	 */
	TestListener(CSI_Spectrum_Analyzer csi_Spectrum_Analyzer) {
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (b == this.csi_Spectrum_Analyzer.miScaleCounts) {
			...
			if (fitType.equals("No Fit")) { // Set fit state
				this.csi_Spectrum_Analyzer.state.setFit(SpectrumData.NO_FIT); // to combo box selection
			} else if (fitType.equals("Constant")) {
				this.csi_Spectrum_Analyzer.state.setFit(SpectrumData.CONSTANT_FIT);
			} else if (fitType.equals("Linear")) {
				this.csi_Spectrum_Analyzer.state.setFit(SpectrumData.LINEAR_FIT);
			} else if (fitType.equals("Exponential")) {
				this.csi_Spectrum_Analyzer.state.setFit(SpectrumData.EXPONENTIAL_FIT);
			} else if (fitType.equals("Power")) {
				this.csi_Spectrum_Analyzer.state.setFit(SpectrumData.POWER_FIT);
			} else if (fitType.equals("LCPL")) {
				this.csi_Spectrum_Analyzer.state.setFit(SpectrumData.LCPL_FIT);
			}
			this.csi_Spectrum_Analyzer.state.updateProfile();
		}
	}
